...
def test_solar_deficit_with_grid_connected_rests_the_battery():
    ...
    # itself is the problem (see the unstable-grid test below). Charging
    # from grid was tried as an opportunistic top-up and then deliberately
    # reverted: NEPA-style supply is often scarce/metered/rationed, so
    # pulling extra current just because grid happens to be up isn't
    # something to do casually — grid-charging now only happens in a
    # genuine low-SOC emergency (see the below-healthy-min tests).
    engine = make_engine()
    state = make_state(
