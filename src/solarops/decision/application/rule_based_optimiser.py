...
class RuleBasedOptimiser:
    ...
    def _battery_health_candidate(self, context: DecisionContext) -> _Candidate | None:
        ...
        if soc < self._config.battery_healthy_min_soc_pct:
            # Emergency-only grid fallback: charge from solar surplus when
            # there is any; otherwise a modest trickle from grid, but only
            # once SOC is genuinely low — not an opportunistic top-up just
            # because grid happens to be available. NEPA-style supply is
            # often scarce/rationed/metered, so pulling extra current beyond
            # what the house needs isn't something to do casually every time
            # the grid is up (reversed after initially building the
            # opportunistic version — see git history for that attempt).
