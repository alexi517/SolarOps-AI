"""BuildingLoadForecaster — Doc 8 §6.2 domain service. Mirrors ``SolarGenerationForecaster``."""
...
__all__ = ["BuildingLoadForecaster"]


class BuildingLoadForecaster:
    def __init__(
        self,
    ...
    def forecast(self, current: EnergyState) -> tuple[Forecast, ForecastGenerated]:
        model = self._registry.get_current(ForecastKind.BUILDING_LOAD)
        if model is None:
            raise NoRegisteredModel(ForecastKind.BUILDING_LOAD)
        ...
        features = build_features(ForecastKind.BUILDING_LOAD, current, history, self._config)
